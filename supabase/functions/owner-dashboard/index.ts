...
Deno.serve(async (req: Request) => {
  ...
  if (userData.user?.id) {
    const { data: adminRows } = await supabase.from("venue_admins").select("user_id").eq("user_id", userData.user.id).in("role", ["owner", "admin"]).limit(1);
  ...
  try {
    ...
    const responses: any[] = [presence15, presence60, presence24, reports24, checkins24, appSignals24, rewardEvents24, redemptions24, venues, live, recentRedemptions, auditLogs];
