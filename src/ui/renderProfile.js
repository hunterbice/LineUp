...
export function legalCopy(kind) {
  ...
  return { title:"Privacy Policy", sections:[
    ...
    ["Your choices","You can use anonymous display mode, decline location and notifications, change preferences, or delete your account in Profile > Account."],
...
export function renderProfilePageHtml(view, ctx) {
  ...
  const views = {
    ...
    preferences() {
      const n = permissionStatusCopy("notification"), l = permissionStatusCopy("location");
      return `<button class="backBtn" onclick="profileBack()">${svg.back} Profile</button><div class="sectionlabel">PREFERENCES</div><div class="card permission"><b>Campus</b><p class="muted intelCopy">University of Arizona is selected for Early Access. Campus selection works without location permission.</p><div class="rewardRule"><b>Selected campus</b><span>University of Arizona</span></div></div><div class="card permission"><b>Notifications</b><p class="muted intelCopy">Get saved venue updates, new deals, and important LineUp alerts.</p><div class="locationStatus"><b>${esc(n.label)}</b><span>${esc(n.detail)}</span></div><button class="secondaryBtn" onclick="openPermissionEducation('notifications')">Review Notification Access</button></div><div class="card permission"><b>Location</b><p class="muted intelCopy">Location improves nearby venue reads while LineUp is open. Campus browsing works without it.</p><div class="locationStatus"><b>${esc(l.label)}</b><span>${esc(l.detail)}</span></div><button class="secondaryBtn" onclick="openPermissionEducation('location')">Review Location Access</button></div>`;
    },
    rewards() {
