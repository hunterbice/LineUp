...
export function mapPoint(coords) {
  coords = coords || {};
  const lat = Number(coords.lat) || 32.2292;
  const lng = Number(coords.lng) || -110.9612;
  const minLat=32.2205,maxLat=32.2370,minLng=-110.9710,maxLng=-110.9440;
  return { x:9+((lng-minLng)/(maxLng-minLng))*82, y:10+((maxLat-lat)/(maxLat-minLat))*78 };
}

export function fallbackMapHtml({ bars, userCoords, colors }) {
  bars = Array.isArray(bars) ? bars.filter(Boolean) : [];
  userCoords = userCoords || [-110.9612, 32.2292];
  colors = colors || {};
  ...
  const pins = bars.map((bar) => {
    ...
    return `<button class="pin" aria-label="${esc(bar.name||"Venue")}" onclick="trackAppEvent('${bar.id}','map_pin_tap',{map:'fallback'});openDetail('${bar.id}')" style="left:${point.x}%;top:${point.y}%;background:${colors[bar.lvl]||"var(--brand)"}"><span></span></button><div class="maplabel" style="left:${point.x+3}%;top:${point.y+5}%">${esc(short)}</div>`;
  }).join("");
...
export function circlePolygon(center,radiusMeters,points) {
  center = center || [-110.9612, 32.2292];
  radiusMeters = Number(radiusMeters) || 230;
  points = Number(points) || 64;
...
export function createVenueMarker({ mapboxgl, map, bar, level, escHtml, onTap }) {
  if (!mapboxgl || !map || !bar || !bar.coords) return null;
  ...
  return new mapboxgl.Marker({element:el,anchor:"bottom"}).setLngLat([Number(bar.coords.lng)||-110.9612,Number(bar.coords.lat)||32.2292]).setPopup(popup).addTo(map);
}
